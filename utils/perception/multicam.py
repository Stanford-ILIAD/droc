...
if __name__ == "__main__":
    # import pickle
    # bgr_images = pickle.load(open('log/shelf/test/0/bgr_images_1685318815.0099695.pkl', 'rb'))
    # depth_images = pickle.load(open('log/shelf/test/0/depth_images_1685318815.0099695.pkl', 'rb'))
    # compute_pcd_offline(['042222070680', '145422071576', '241222076578'], bgr_images, depth_images)
    # exit()

    multi_cam = MultiCam(['042222070680', '145422071576', '241222076578'])
    # while True:
    #     # bgr_images, _ = multi_cam.take_raw_bgrd()
    #     bgr_images = multi_cam.take_bgr()
    #     # image = bgr_images['145422071576']
    #     cv2.imshow('RGB', np.concatenate(list(bgr_images.values()), axis=1))
    #     if cv2.waitKey(1) & 0xFF == 27:
    #         break
    # cv2.destroyAllWindows()

    while True:
        rgb_images, pcd_merged, _, _ = multi_cam.take_bgrd()
    ...
    # start = time.time()
    # bgr_images, pcd_merged = multi_cam.take_bgrd()
    # end = time.time()

    # waypoints_proj = multi_cam.project_robot_waypoint(ee_pos)

    # for cam in multi_cam.cameras:
    #     img = bgr_images[cam.serial_no]
    # #     u,v = waypoints_proj[cam.serial_no]
    # #     cv2.circle(img, (u,v), 3, (0,0,255), -1)
    #     cv2.imwrite('%s.png'%cam.serial_no, img)
    #     # cv2.imshow('img', img)
    #     # cv2.waitKey(0)

    # print(waypoints_proj)

    #### Calibration ####
    # multi_cam = MultiCam(['042222070680', '241222076578'])
    # multi_cam.calibrate_front_cams()
    # multi_cam = MultiCam(['145422071576'])
    # multi_cam.calibrate_back_cam()
    # multi_cam = MultiCam(['042222070680', '145422071576', '241222076578'])
    # for i in range(3):
    #     multi_cam.take_bgrd()
